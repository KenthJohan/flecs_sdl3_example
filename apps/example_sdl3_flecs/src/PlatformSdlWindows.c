...
static void System_EgKeyboardsState(ecs_iter_t *it)
{
	...
	for (int i = 0; i < EG_KEYBOARDS_KEYS_MAX; i++) {
		s->state[i] &= ~EG_KEYBOARDS_STATE_PRESSED;
	}
	while (SDL_PollEvent(&event)) {
		// SDLTest_CommonEvent(state, &event, &done);
		switch (event.type) {
		case SDL_EVENT_KEY_DOWN:
			if (s->state[event.key.scancode] & EG_KEYBOARDS_STATE_DOWN) {
				//s->state[event.key.scancode] &= ~EG_KEYBOARDS_STATE_RISING_EDGE;
			} else {
		...
		case SDL_EVENT_WINDOW_RESIZED: {
			...
			ecs_entity_t e = ev[0];
			ecs_trace("SDL_EVENT_WINDOW_RESIZED: %s\n", ecs_get_name(it->world, e));
...
void PlatformSdlWindowsImport(ecs_world_t *world)
{
	ECS_MODULE(world, PlatformSdlWindows);
	ecs_set_name_prefix(world, "PlatformSdlWindows");
